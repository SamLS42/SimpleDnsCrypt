...
public class LoaderViewModel : Screen
{
	...
	public LoaderViewModel(IWindowManager windowManager, IEventAggregator events)
	{
		...
		System.Collections.ObjectModel.ObservableCollection<Language> languages = LocalizationEx.GetSupportedLanguages();
		...
		_systemTrayViewModel = new SystemTrayViewModel(_windowManager, _mainViewModel);
	...
	private async Task<string> StartRemoteUpdateDownload(RemoteUpdate remoteUpdate)
	{
		...
		try
		{
			...
			if (!string.IsNullOrEmpty(signature) && installer != null)
			{
				...
				minisign.Models.MinisignSignature loadedSignature = Minisign.LoadSignature(Convert.FromBase64String(s[1]), trustedCommentBinary,
					Convert.FromBase64String(s[3]));
				minisign.Models.MinisignPublicKey publicKey = Minisign.LoadPublicKeyFromString(Global.ApplicationUpdatePublicKey);
