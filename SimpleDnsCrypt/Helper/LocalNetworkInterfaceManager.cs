...
public static class LocalNetworkInterfaceManager
{
	...
	public static bool IsUsingDnsCrypt(List<string> listenAddresses, LocalNetworkInterface localNetworkInterface, bool strictCheck = false)
	{
		...
		List<string> addressOnly = [.. from listenAddress in listenAddresses let lastIndex = listenAddress.LastIndexOf(":", StringComparison.Ordinal) select listenAddress.Substring(0, lastIndex).Replace("[", "").Replace("]", "")];

		if (strictCheck)
		{
			// check for ALL addresses
			return addressOnly.Intersect([.. localNetworkInterface.Dns.Select(x => x.Address)]).Count() == addressOnly.Count;
		}
